...
describe('when reduce clients', () => {
    ...
    describe('when delete existing client', () => {
        ...
        const action = {
            type: actions.DELETE_CLIENT_SUCCEEDED,
            payload: '1'
        };

        const expectedRetult = [clients[1], clients[2]];
        const actualResult = reducer(state, action).clients;
    ...
    describe('when delete a client which is not exist', () => {
        ...
        const expectedRetult = clients;
        const reducerRes = reducer(state, action);
        console.log(reducerRes);
        const actualResult = reducerRes.clients;
